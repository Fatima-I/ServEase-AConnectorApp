...
import 'user_dashboard_screen.dart'; // Ensure correct User navigation
...
class _GeneralInfoScreenState extends State<GeneralInfoScreen> {
  ...
  // --- SAVE FUNCTION UPDATED FOR ADMIN APPROVAL LOGIC ---
        ...
        if (mounted) {
          showDialog(
            ...
            builder: (ctx) => AlertDialog(
              ...
              content: const Text("Your worker profile has been sent to the Admin for approval. You can login once approved."),
              actions: [
                TextButton(
                  onPressed: () {
                    Navigator.pop(ctx);
                  ...
                  child: const Text("OK", style: TextStyle(color: lightSeaGreen)),
                )
              ],
            ),
          );
  ...
  Widget _buildAccountTypeSelection() {
    return Column(
      children: [
        ...
        const SizedBox(height: 20),
        const Text("Choose Account Type", style: TextStyle(fontSize: 22, fontWeight: FontWeight.bold, color: Colors.black87)),
        const SizedBox(height: 8),
        const Text("Are you registering as a User or a Worker?", textAlign: TextAlign.center, style: TextStyle(fontSize: 15, color: Colors.black54)),
        const SizedBox(height: 30),
        ElevatedButton.icon(
          icon: const Icon(Icons.person_outline, color: Colors.white),
          style: ElevatedButton.styleFrom(backgroundColor: lightSeaGreen, minimumSize: const Size(double.infinity, 55), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(14))),
          onPressed: () => setState(() => _isWorker = false),
          label: const Text("Register as User", style: TextStyle(color: Colors.white, fontSize: 16)),
        ),
        const SizedBox(height: 20),
        ElevatedButton.icon(
          icon: const Icon(Icons.handyman_rounded, color: Colors.white),
          style: ElevatedButton.styleFrom(backgroundColor: Colors.deepOrangeAccent, minimumSize: const Size(double.infinity, 55), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(14))),
          onPressed: () => setState(() => _isWorker = true),
          label: const Text("Register as Worker", style: TextStyle(color: Colors.white, fontSize: 16)),
        ),
      ],
    );
  }
